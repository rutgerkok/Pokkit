...
public final class PokkitPluginManager implements PluginManager {
    ...
    public Set<Permission> getDefaultPermissions(boolean op) {
        return nukkit.getDefaultPermissions(op)
                .values()
                .stream()
                .map(PokkitPermission::toBukkit)
                .collect(Collectors.toSet());
    }

    @Override
    public Set<Permissible> getDefaultPermSubscriptions(boolean op) {
        return nukkit.getDefaultPermSubscriptions(op)
                .stream()
                .map(PokkitPermissible::toBukkit)
                .collect(Collectors.toSet());
    }
    ...
    public Set<Permissible> getPermissionSubscriptions(String permission) {
        return nukkit.getPermissionSubscriptions(permission)
                .stream()
                .map(PokkitPermissible::toBukkit)
                .collect(Collectors.toSet());
    }
    ...
    public Plugin loadPlugin(File file)
        ...
        // Load the plugin
        Map<String, cn.nukkit.plugin.PluginLoader> pluginLoaders = Collections.singletonMap(
                PokkitPluginLoader.class.getName(), this.pluginLoader);
